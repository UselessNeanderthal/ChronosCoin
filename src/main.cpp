...
#if defined(NDEBUG)
# error "Chronoscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x65c275d7ef1efcf723331a786f5f95c7d54b870506fdddd5f2befe24b266d877");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Chronoscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Chronoscoin Signed Message:\n";
...
{
    // Chronoscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 200000); // Chronoscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.5 * 24 * 60 * 60; // Chronoscoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Chronoscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Chronoscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Chronoscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x349cd6bb99d4ba9b364cd5be24aaac7e63a85afe0f91a29a14989403372fe3d0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "USA Today 26/Feb/2022 Russian troops enter Ukraine's second largest city";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0487866b318e62e981fb4fcc974454c4630e13de8228a009650c4b8c675d08ea43adf094a062799c2bb29bca7d413d52836c66fb2cc219ef088b8b152735e8ba0f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1645979698;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086074884;

        if (fTestNet)
        {
            block.nTime    = 1645979667;
            block.nNonce   = 385835180;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x39c0e01edc93f945a32d063bbabfe3b68e9979a12d0e68b27a888d1d538279d0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Chronoscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ChronoscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChronoscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ChronoscoinMiner(CWallet *pwallet)
{
    printf("ChronoscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("chronoscoin-miner");
    ...
    try { loop {
        ...
        printf("Running ChronoscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ChronoscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ChronoscoinMiner, pwallet));
}
